...
class ModelArguments:
    ...
    scl_temperature: Optional[float] = field(default=0.3)
...
class LazySupervisedClassificationDataset(Dataset):
    ...
    def lengths(self):
        length_list = []
        for sample in self.list_data_dict:
            img_tokens = 144 if 'image' in sample else 0
            length_list.append(sum(len(conv['value'].split()) for conv in sample['text']) + img_tokens)
        return length_list
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        input_ids = tokenizer_image_token(sources["text"], self.tokenizer, return_tensors='pt')

        if isinstance(sources["label"], str):
            labels = torch.LongTensor([sources["label"] == "positive"])
        elif isinstance(sources["label"], int):
            labels = torch.LongTensor([sources["label"]])
        elif isinstance(sources["label"], float):
            labels = torch.FloatTensor([sources["label"]])
...
def make_supervised_classification_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazySupervisedClassificationDataset(tokenizer=tokenizer,
                                data_path=data_args.data_path,
                                data_args=data_args)
    val_dataset = LazySupervisedClassificationDataset(tokenizer=tokenizer,
...
def compute_metrics(eval_preds):
    ...
    if logits.shape[-1] >= 2:
        ...
        predictions = np.array([int(round(max(0, min(s.item(), 3)))) for s in logits])
        ...
        f1 = metric3.compute(predictions=predictions, references=labels,
                            average="macro")["f1"]
        print(metric3.compute(predictions=predictions, references=labels, average=None))
    ...
    elif labels.dtype in [np.float16, np.float32, np.float64]:
    
        metric1 = evaluate.load("mse")

        return metric1.compute(predictions=logits, references=labels)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
        ...
        elif 'phi-2' in model_args.model_name_or_path.lower():
            model = LlavaPhi3ForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
                scl_temperature=model_args.scl_temperature,
                loss_weight_scalar=model_args.loss_weight_scalar,
                **bnb_model_from_pretrained_args
            )
        elif 'phi-3' in model_args.model_name_or_path.lower():
            model = LlavaPhi3Classifier.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            model = LlavaGemmaClassifier.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
    ...
    elif model_args.version == "phi_2_instruct":
        if tokenizer.pad_token is None:
            smart_tokenizer_and_embedding_resize(
                special_tokens_dict=dict(pad_token="[PAD]"),
                tokenizer=tokenizer,
    ...
    data_module = make_supervised_classification_data_module(tokenizer=tokenizer, data_args=data_args)
