...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        for pkl_data_path in data_path.split(":"):
            subset_examples = glob.glob(os.path.join(pkl_data_path, '**', '*.pkl'), recursive=True)
            print("{} contains {} examples".format(pkl_data_path, len(subset_examples)))
            if "obelics" in pkl_data_path.lower():
                self.list_data_dict += subset_examples[:data_args.train_num_samples//2]
            elif "datacomp" in pkl_data_path.lower():
                self.list_data_dict += subset_examples[:data_args.train_num_samples//2]
            else:
                self.list_data_dict += subset_examples
            print("Now the dataset is expanded to contain {} examples".format(len(self.list_data_dict)))
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        with open(self.list_data_dict[i], 'rb') as f:
            data_dict = pickle.load(f)
        image_tensors = [self.data_args.image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0] for image in data_dict['image_tensors']]
...
def train(attn_implementation=None):
    ...
    if model_args.version == "v0":
        ...
        if "llama-3" in model_args.model_name_or_path.lower():
            ...
            tokenizer.add_special_tokens({"additional_special_tokens": ["<s>", "<|pad|>"]})
