...
class LlavaLlamaClassifier(LlamaForSequenceClassification, LlavaMetaForCausalLM):
    config_class = LlavaConfig
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # calculating croos-entropy loss for classification learning
            classification_loss_fct = nn.CrossEntropyLoss()
            classification_loss = classification_loss_fct(classification_logits.view(-1, self.num_labels), labels.view(-1))
            
            # calculating contrastive loss for contrastive learning
            scl_loss_fct = SupervisedContrastiveLoss(self.scl_temperature)
            scl_loss = scl_loss_fct(pooled_logits, labels.view(-1))
            
            if scl_loss.device.index == "cuda:0":
                print(classification_loss)
            
            loss = classification_loss #(self.loss_weight_scalar * scl_loss) + (1 - self.loss_weight_scalar) * (classification_loss)
